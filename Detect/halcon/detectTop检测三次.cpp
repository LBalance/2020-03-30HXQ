...
// Local procedures 
void detectTop (HObject ho_Image, HTuple hv_WindowHandle, HTuple hv_input_Scale_1, 
    HTuple hv_input_Scale_2, HTuple hv_input_Scale_3, HTuple hv_input_Scale_4, HTuple hv_BaoLuoDiameterUp, 
    HTuple hv_BaoLuoDiameterDown, HTuple hv_GouWidthUp, HTuple hv_GouWidthDown, HTuple *hv_Result);
...
void detectSide (HObject ho_Image, HTuple hv_WindowHandle, HTuple hv_input_Scale_1Goupian, 
    HTuple hv_input_Scale_2, HTuple hv_input_Scale_3, HTuple hv_input_Scale_4, HTuple hv_CaoWidthUp, 
    HTuple hv_CaoWidthDown, HTuple hv_MaociWidth, HTuple hv_MaociHeight, HTuple *hv_Result);
    ...
    HTuple hv_BaoLuoDiameterDown, HTuple hv_GouWidthUp, HTuple hv_GouWidthDown, HTuple *hv_Result)
{
  ...
  HObject  ho_imageCorrected2, ho_imageCorrectedSTD, ho_region4;
  HObject  ho_gouTopRegions, ho_region2, ho_region3;

  // Local control variables
  HTuple  hv_hwnd, hv_ret1, hv_ret2, hv_ret3, hv_ret4;
  HTuple  hv_ret5;
  ...
  set_display_font(hv_WindowHandle, 11, "mono", "false", "false");

  funCorrectImage(ho_Image, &ho_imageCorrected0, &ho_imageCorrected1, &ho_imageCorrected2
      );

  if (HDevWindowStack::IsOpen())
    DispObj(ho_imageCorrected0, HDevWindowStack::GetActive());
  ...
  hv_ret5 = 0;
  //检测槽内异物，堵槽不检
  //fun1 (imageCorrected1, region1, hwnd, ret1)

  //外径跳动检测,找到非畸变图片
  fun5(ho_imageCorrected0, ho_imageCorrected1, ho_imageCorrected2, &ho_imageCorrectedSTD, 
      hv_hwnd, hv_input_Scale_1, hv_BaoLuoDiameterDown, hv_BaoLuoDiameterUp, &hv_ret5);

  if (0 != (hv_ret5==0))
  {
    //包络线直径
    fun4(ho_imageCorrectedSTD, &ho_region4, hv_hwnd, hv_input_Scale_1, hv_input_Scale_4, 
        hv_BaoLuoDiameterDown, hv_BaoLuoDiameterUp, &hv_ret4);

    //钩宽，钩变形
    fun2(ho_imageCorrectedSTD, &ho_gouTopRegions, &ho_region2, hv_hwnd, hv_GouWidthDown, 
        hv_GouWidthUp, &hv_ret2);

    //钩宽不合格，钩间距离检测无意义，直接跳过。在外径不合格时也跳过。
    if (0 != (HTuple(hv_ret2==0).TupleAnd(hv_ret4==0)))
    {
      //钩不居中、钩偏，在堵槽情况下，模拟套环来检测
      fun3(ho_imageCorrectedSTD, ho_gouTopRegions, &ho_region3, hv_hwnd, hv_input_Scale_2, 
          hv_input_Scale_3, &hv_ret3);
    }
  }
  (*hv_Result) = 0;
  ...
  {
    set_display_font(hv_WindowHandle, 20, "mono", "false", "false");
    disp_message(hv_hwnd, "OK-良品", "window", 40, 30, "blue", "true");
  }
  set_display_font(hv_WindowHandle, 11, "mono", "false", "false");

  disp_message(200000, ((((((((((((((("argv:"+hv_input_Scale_1)+" ")+hv_input_Scale_2)+" ")+hv_input_Scale_3)+" ")+hv_input_Scale_4)+" ")+hv_BaoLuoDiameterUp)+" ")+hv_BaoLuoDiameterDown)+" ")+hv_GouWidthUp)+" ")+hv_GouWidthDown)+" ", 
      "window", 0, 200, "black", "true");

  disp_message(200000, (((((("粘"+hv_input_Scale_1)+"宽")+hv_input_Scale_1)+"中")+hv_input_Scale_1)+"经")+hv_input_Scale_1, 
...
void fun1 (HObject ho_Image, HObject *ho_region1, HTuple hv_hwnd, HTuple *hv_ret1)
{
  ...
  if (HDevWindowStack::IsOpen())
    DispObj(ho_RegionDilation1, HDevWindowStack::GetActive());
  if (HDevWindowStack::IsOpen())
    SetLineWidth(HDevWindowStack::GetActive(),1);
  //hv_hwnd
  return;
    ...
    HTuple hv_input_Scale_4, HTuple hv_gouchangMin, HTuple hv_gouchangMax, HTuple *hv_ret4)
{
  ...
  if (HDevWindowStack::IsOpen())
    DispObj(ho_CircleSTD1, HDevWindowStack::GetActive());
  ...
  for (hv_Index=1; hv_Index.Continue(end_val85, step_val85); hv_Index += step_val85)
  {
    ...
    else
    {
      ...
      if (HDevWindowStack::IsOpen())
        DispObj(ho_diameterRegion1, HDevWindowStack::GetActive());
    }
  ...
  if (HDevWindowStack::IsOpen())
    DispObj((*ho_region4), HDevWindowStack::GetActive());
  //hv_hwnd
  return;
    ...
    HTuple hv_GouTaoWide, HTuple hv_GouGouTaoIntersectionInner_radius, HTuple *hv_ret3)
{
  ...
  for (hv_Index=0; hv_Index<=1.2; hv_Index+=0.05)
  {
    ...
    if (0 != hv_isDebugMark)
    {
      ...
      if (HDevWindowStack::IsOpen())
        DispObj(ho_gouTaoNow, HDevWindowStack::GetActive());
      if (HDevWindowStack::IsOpen())
        SetColor(HDevWindowStack::GetActive(),"red");
  ...
  if (0 != (HTuple(hv_errorRgnArea>0).TupleAnd((*hv_ret3)==0)))
  {
    ...
    if (HDevWindowStack::IsOpen())
      SetColor(HDevWindowStack::GetActive(),"red");
    if (HDevWindowStack::IsOpen())
      DispObj((*ho_region3), HDevWindowStack::GetActive());
  }
  //hv_hwnd
  return;
}
    ...
    HTuple hv_goukuanMin, HTuple hv_goukuanMax, HTuple *hv_ret2)
{
  ...
  if (HDevWindowStack::IsOpen())
    SetColor(HDevWindowStack::GetActive(),"red");
  if (HDevWindowStack::IsOpen())
    DispObj((*ho_region2), HDevWindowStack::GetActive());
  //hv_hwnd
  return;
}
    ...
    HTuple hv_CaoWidthDown, HTuple hv_MaociWidth, HTuple hv_MaociHeight, HTuple *hv_Result)
{
  ...
  // Local iconic variables
  HObject  ho_ImagePJ, ho_region1, ho_outRegion1;
  HObject  ho_region2, ho_region3, ho_region4;

  // Local control variables
  HTuple  hv_ret1, hv_ret2, hv_ret3, hv_ret4, hv_DownRow;
  HTuple  hv_ret5, hv_hwnd;
  ...
  SideCHH_PingJie(ho_Image, &ho_ImagePJ, 1080, 2, 0, 200, &hv_DownRow);
  ...
  hv_hwnd = 200000;
  if (HDevWindowStack::IsOpen())
    DispObj(ho_ImagePJ, HDevWindowStack::GetActive());

  disp_message(200000, (((((((((((((("argv:"+hv_input_Scale_1Goupian)+" ")+hv_input_Scale_2)+" ")+hv_input_Scale_3)+" ")+hv_input_Scale_4)+" ")+hv_CaoWidthUp)+" ")+hv_CaoWidthDown)+" ")+hv_MaociWidth)+" ")+hv_MaociHeight, 
      "window", 0, 200, "black", "true");
  disp_message(200000, (((((((("宽"+hv_input_Scale_1Goupian)+"粘")+hv_input_Scale_1Goupian)+"碰")+hv_input_Scale_1Goupian)+"高")+hv_input_Scale_1Goupian)+"中")+hv_input_Scale_1Goupian, 
      "window", 20, 200, "black", "true");

  //槽宽、槽连续
  Sidefun1(ho_ImagePJ, &ho_region1, &ho_outRegion1, hv_WindowHandle, hv_CaoWidthDown, 
      hv_CaoWidthUp, &hv_ret1);
  if (0 != (HTuple(hv_ret1==0).TupleOr(hv_ret1==-10)))
  {
    //毛刺、粘铜
    Sidefun2(ho_ImagePJ, ho_outRegion1, &ho_region2, hv_WindowHandle, hv_MaociWidth, 
        hv_MaociHeight, &hv_ret2);

    //片区大划伤、生锈
    Sidefun3(ho_ImagePJ, ho_outRegion1, &ho_region3, hv_WindowHandle, hv_input_Scale_2, 
        &hv_ret3);

    //钩顶到换向器底部距离（钩右极限位置）  + 钩偏（返回值：钩高度不良ret4<0, 钩偏ret5<0）
    Sidefun4(ho_ImagePJ, ho_outRegion1, &ho_region4, hv_input_Scale_1Goupian, hv_WindowHandle, 
        &hv_ret4, &hv_ret5);

    //对钩居中的检测（肩上方区域黑色宽度）
    //Sidefun5 (ImagePJ, outRegion1, region5, WindowHandle, input_Scale_1Goupian, ret5)
  }
  //**此处第二版粘铜区分下料注意注意：同时有多种不良时最后反馈结果为粘铜
  //**2个工位检测有多种不良结果时，最后反应为粘铜
  (*hv_Result) = 0;
    ...
    HTuple hv_CaoWidthMin, HTuple hv_CaoWidthMax, HTuple *hv_ret1)
{
  ...
  HObject  ho_RegionOpening5, ho_ConnectedRegions1, ho_SelectedRegions1;
  HObject  ho_RegionFillUp1, ho_RegionOpening2, ho_Partitioned;
  ...
  for (hv_Index=1; hv_Index<=23; hv_Index+=1)
  {
    ...
    Threshold(ho_ImageReduced4, &ho_Regions4, 0, 65);
    ClosingRectangle1(ho_Regions4, &ho_RegionClosing3, 20, 1);
    OpeningCircle(ho_RegionClosing3, &ho_RegionOpening5, 3.5);
    ...
    PartitionRectangle(ho_RegionOpening2, &ho_Partitioned, 20, 200);
    ...
    //对宽度缩放,边缘不反光
    hv_addoffset = 20;
    hv_caoHeightValuesMax_mm = (hv_caoHeightValuesMax-hv_addoffset)*0.010382;
    hv_caoHeightValuesMin_mm = (hv_caoHeightValuesMin-hv_addoffset)*0.010382;
    hv_caoHeightValuesMean_mm = (hv_caoHeightValuesMean-hv_addoffset)*0.010382;
    ...
    HTuple hv_MaociWidth, HTuple hv_MaociHeight, HTuple *hv_ret2)
{
  ...
  HObject  ho_ObjectSelectedSK, ho_RegionErosion, ho_RegionMiddle;
  HObject  ho_RegionDifference2, ho_ConnectedRegions3, ho_SelectedRegionsRight;
  HObject  ho_SelectedRegionsLeft, ho_ImageReduced3, ho_ImageMedian;
  HObject  ho_ImageMedian1, ho_Regions2, ho_RegionClosing1;
  HObject  ho_RegionOpening1, ho_ConnectedRegions2, ho_maxHeightRrgion;
  HObject  ho_RegionTrans1, ho_RegionDilation1, ho_RegionDifference;
  HObject  ho_ImageReduced5, ho_Regions4, ho_ConnectedRegions4;
  HObject  ho_maxHeightRWhite, ho_ImageReduced4, ho_Regions3;
  ...
  HTuple  hv_Value1, hv_Value8, hv_ObjectSelectedSKheight;
  HTuple  hv_Mean, hv_Deviation, hv_Mean1, hv_Deviation1;
  HTuple  hv_thresholdNum, hv_heights114, hv_heightMax115;
  HTuple  hv_Value4, hv_Max1, hv_Value3, hv_errorRgnArea;
  ...
  for (hv_Index=1; hv_Index<=22; hv_Index+=1)
  {
    ...
    for (hv_Index2=1; hv_Index2.Continue(end_val46, step_val46); hv_Index2 += step_val46)
    {
      ...
      if (0 != (HTuple(HTuple(hv_Value[3])>400).TupleOr(HTuple(hv_Value[1])>40)))
      {
        ...
        RegionFeatures(ho_ObjectSelected2, (HTuple("width").Append("height")), &hv_Value8);
        if (0 != (HTuple(HTuple(hv_Value8[0])<50).TupleAnd(HTuple(hv_Value8[0])<40)))
        {
          continue;
        }
    ...
    //胜克电木粉提取
    ErosionRectangle1(ho_ObjectSelected, &ho_ObjectSelectedSK, 15, 15);
    RegionFeatures(ho_ObjectSelectedSK, "height", &hv_ObjectSelectedSKheight);
    ErosionRectangle1(ho_ObjectSelectedSK, &ho_RegionErosion, 350, 0.5);
    MoveRegion(ho_RegionErosion, &ho_RegionMiddle, 0, -30);
    Difference(ho_ObjectSelectedSK, ho_RegionMiddle, &ho_RegionDifference2);
    ...
    SelectShape(ho_ConnectedRegions3, &ho_SelectedRegionsLeft, "area", "and", 4000, 
        8000);

    //中间段
    ReduceDomain(ho_ImageReduced, ho_RegionMiddle, &ho_ImageReduced3);
    MedianImage(ho_ImageReduced3, &ho_ImageMedian, "square", 3, "mirrored");
    Intensity(ho_RegionMiddle, ho_ImageMedian, &hv_Mean, &hv_Deviation);
    MedianImage(ho_ImageReduced3, &ho_ImageMedian1, "square", 12, "mirrored");
    Intensity(ho_RegionMiddle, ho_ImageMedian1, &hv_Mean1, &hv_Deviation1);
    if (0 != (hv_Deviation>8.5))
    {
      {
      HTuple end_val110 = hv_Mean-7;
      HTuple step_val110 = 2;
      for (hv_thresholdNum=20; hv_thresholdNum.Continue(end_val110, step_val110); hv_thresholdNum += step_val110)
      {
        Threshold(ho_ImageMedian, &ho_Regions2, 0, hv_thresholdNum);
        ClosingCircle(ho_Regions2, &ho_RegionClosing1, 3.5);
        OpeningCircle(ho_RegionClosing1, &ho_RegionOpening1, 3.5);
        Connection(ho_RegionOpening1, &ho_ConnectedRegions2);
        RegionFeatures(ho_ConnectedRegions2, "height", &hv_heights114);
        TupleMax(hv_heights114, &hv_heightMax115);
        SelectShape(ho_ConnectedRegions2, &ho_maxHeightRrgion, "height", "and", hv_heightMax115-1, 
            hv_heightMax115+1);

        if (0 != (hv_heightMax115>(hv_ObjectSelectedSKheight-10)))
        {
          //排除灰度渐进加深无明显阶梯的情况,有梯度的在黑区附近有高于其15灰度的高度大于30的区域
          if (0 != (hv_Deviation1<8))
          {
            ShapeTrans(ho_maxHeightRrgion, &ho_RegionTrans1, "rectangle1");
            DilationRectangle1(ho_RegionTrans1, &ho_RegionDilation1, 100, 0.5);
            Difference(ho_RegionDilation1, ho_maxHeightRrgion, &ho_RegionDifference
                );
            ReduceDomain(ho_ImageMedian, ho_RegionDifference, &ho_ImageReduced5);
            Threshold(ho_ImageReduced5, &ho_Regions4, hv_thresholdNum+15, 255);
            Connection(ho_Regions4, &ho_ConnectedRegions4);
            RegionFeatures(ho_ConnectedRegions4, "height", &hv_Value4);
            TupleMax(hv_Value4, &hv_Max1);
            if (0 != (hv_Max1<1))
            {
              hv_Max1 = 1;
            }
            SelectShape(ho_ConnectedRegions4, &ho_maxHeightRWhite, "height", "and", 
                hv_Max1-1, hv_Max1+1);
            if (0 != (hv_Max1<30))
            {
              break;
          ...
          ConcatObj(ho_EmptyRegion, ho_maxHeightRrgion, &ho_EmptyRegion);
          break;
        }
      }
      }
    ...
    MedianImage(ho_ImageReduced3, &ho_ImageMedian, "square", 3, "mirrored");
    Intensity(ho_SelectedRegionsRight, ho_ImageMedian, &hv_Mean, &hv_Deviation);
    MedianImage(ho_ImageReduced3, &ho_ImageMedian1, "square", 12, "mirrored");
    Intensity(ho_SelectedRegionsRight, ho_ImageMedian1, &hv_Mean1, &hv_Deviation1);
    if (0 != (hv_Deviation>8.5))
    {
      {
      HTuple end_val153 = hv_Mean-7;
      HTuple step_val153 = 2;
      for (hv_thresholdNum=20; hv_thresholdNum.Continue(end_val153, step_val153); hv_thresholdNum += step_val153)
      {
        Threshold(ho_ImageMedian, &ho_Regions2, 0, hv_thresholdNum);
        ClosingCircle(ho_Regions2, &ho_RegionClosing1, 3.5);
        OpeningCircle(ho_RegionClosing1, &ho_RegionOpening1, 3.5);
    ...
    //中段和底部段联合判断
    //union2 (RegionMiddle, SelectedRegionsRight, MiddleAndButtomRegion)
    //reduce_domain (Image, MiddleAndButtomRegion, ImageReduced5)


    //斜坡处正常情况下为黑色，有异物时灰度增高
    ReduceDomain(ho_Image, ho_SelectedRegionsLeft, &ho_ImageReduced4);
    Threshold(ho_ImageReduced4, &ho_Regions3, 60, 255);
    RegionFeatures(ho_Regions3, "area", &hv_Value3);
    if (0 != (hv_Value3>3500))
    {
  ...
  if (0 != (HTuple(hv_errorRgnArea==0).TupleAnd((*hv_ret3)==0)))
  {
    ...
    if (0 != (hv_Max>50))
    {
      ...
      if (HDevWindowStack::IsOpen())
        SetColor(HDevWindowStack::GetActive(),"red");
      if (HDevWindowStack::IsOpen())
        DispObj(ho_PianErrDouble, HDevWindowStack::GetActive());
      return;
    }
  }
...
void funMatchDifference (HObject ho_Image, HTuple hv_hwnd)
{
  ...
  HTuple  hv_Column0, hv_areaValue, hv_Number, hv_Area, hv_Row1;
  HTuple  hv_Column1, hv_Row, hv_Column, hv_Angle, hv_Score;
  HTuple  hv_Matrix1, hv_Matrix3, hv_Matrix4;
  ...
  if (0 != hv_isCreate)
  {
    //read_image (Image0, 'E:/换向器/Image/Top/Bad/0078.tif')
    //gen_rectangle1 (ROI_0, 54.5, 1682.5, 620.5, 2112.5)
    ...
    //write_shape_model (ModelID, 'E:/换向器/halcon/mode.shm')

    //threshold (Image0, Regions, 0, 200)
    //opening_circle (Regions, RegionOpening, 5)
    //connection (RegionOpening, ConnectedRegions)
    //select_shape_std (ConnectedRegions, SelectedRegions, 'max_area', 70)
    //write_region (SelectedRegions, 'E:/换向器/halcon/stdreg.reg')

  }

  ReadShapeModel("E:/换向器/halcon/mode.shm", &hv_ModelID);

  ReadRegion(&ho_RegionSTD, "E:/换向器/halcon/stdreg.reg");
  ...
  if (HDevWindowStack::IsOpen())
    DispObj(ho_RegionDilation2, HDevWindowStack::GetActive());
  if (HDevWindowStack::IsOpen())
    DispObj(ho_RegionDilation3, HDevWindowStack::GetActive());
  return;
}
    ...
    HTuple hv_hwnd, HTuple *hv_ret4, HTuple *hv_ret5)
{
  ...
  if (HDevWindowStack::IsOpen())
    DispObj(ho_RegionLinesSTD, HDevWindowStack::GetActive());
    ...
    else
    {
      ...
      if (0 != (HTuple(hv_SingleGouDingHeight>110).TupleOr(hv_SingleGouDingHeight<70)))
      {
        ...
        if (HDevWindowStack::IsOpen())
          SetColor(HDevWindowStack::GetActive(),"red");
        if (HDevWindowStack::IsOpen())
          DispObj(ho_gouDingSingle, HDevWindowStack::GetActive());
        return;
      }
    ...
    HTuple hv_shoulderValue, HTuple *hv_ret5)
{
  ...
  if (HDevWindowStack::IsOpen())
    SetColor(HDevWindowStack::GetActive(),"red");
  if (HDevWindowStack::IsOpen())
    DispObj((*ho_region5), HDevWindowStack::GetActive());
  //hv_hwnd
  return;
    ...
    HTuple hv_ColumnCoordinate, HTuple hv_theTupleTobeShow)
{
  ...
  //放大显示
  if (HDevWindowStack::IsOpen())
    SetPart(HDevWindowStack::GetActive(),hv_Row1, hv_Column1, hv_Row2/5, hv_Column2/10);
  ...
  if (HDevWindowStack::IsOpen())
    SetDraw(HDevWindowStack::GetActive(),"margin");
  if (HDevWindowStack::IsOpen())
    SetColor(HDevWindowStack::GetActive(),hv_showColor);
  ...
  if (HDevWindowStack::IsOpen())
    DispObj(ho_RegionMoved, HDevWindowStack::GetActive());

  //复位
  if (HDevWindowStack::IsOpen())
    SetPart(HDevWindowStack::GetActive(),hv_Row1, hv_Column1, hv_Row2, hv_Column2);
  //hv_hwnd
  return;
}
...
void action()
{
  ...
  // Local control variables
  HTuple  hv_BaoluoDiameterUp, hv_BaoluoDiameterDown;
  HTuple  hv_GouWidthUp, hv_GouWidthDown, hv_input_Scale_1;
  HTuple  hv_input_Scale_2, hv_input_Scale_3, hv_input_Scale_4;
  HTuple  hv_CaoWidthUp, hv_CaoWidthDown, hv_MaoCiWidth, hv_MaoCiHeight;
  HTuple  hv_input_Scale_1s, hv_input_Scale_2s, hv_input_Scale_3s;
  HTuple  hv_input_Scale_4s, hv_WindowHandle, hv_ImageFiles;
  HTuple  hv_Length, hv_Index, hv_Result;


  hv_BaoluoDiameterUp = 28.03;
  hv_BaoluoDiameterDown = 27.67;
  hv_GouWidthUp = 1.2;
  hv_GouWidthDown = 1.0;
  hv_input_Scale_1 = 0.997;
  hv_input_Scale_2 = 56;
  hv_input_Scale_3 = 2.5;
  hv_input_Scale_4 = 15;
  //27.85/2373=0.011736

  hv_CaoWidthUp = 0.62;
  hv_CaoWidthDown = 0.48;
  hv_MaoCiWidth = 0.15;
  hv_MaoCiHeight = 0.15;
  hv_input_Scale_1s = 15;
  hv_input_Scale_2s = 5000;
  hv_input_Scale_3s = 1;
  hv_input_Scale_4s = 1;
  //d=23.43  *3.1416/7090=0.010382

  hv_WindowHandle = 200000;
  //'C:/Users/Administrator/MVS/Data'
  //'D:/Images/Top/Bad'
  //'D:/Images/Top/All'
  if (0 != 0)
  {
    list_image_files("D:/Images/Top/Bad", "default", HTuple(), &hv_ImageFiles);
    ...
    for (hv_Index=0; hv_Index.Continue(end_val30, step_val30); hv_Index += step_val30)
    {
      ...
      //检测槽内异物，包络线直径，钩宽，钩变形，钩不居中
      detectTop(ho_Image, hv_WindowHandle, hv_input_Scale_1, hv_input_Scale_2, hv_input_Scale_3, 
          hv_input_Scale_4, hv_BaoluoDiameterUp, hv_BaoluoDiameterDown, hv_GouWidthUp, 
          hv_GouWidthDown, &hv_Result);
      if (0 != (hv_Result==0))
      {
        hv_input_Scale_4 += 1;
      }
  ...
  {
    list_image_files("D:/Images/Side/Bad", "default", HTuple(), &hv_ImageFiles);
    ...
    for (hv_Index=0; hv_Index.Continue(end_val45, step_val45); hv_Index += step_val45)
    {
      ...
      //检测槽内异物，包络线直径，钩宽，钩变形，钩不居中
      detectSide(ho_Image, hv_WindowHandle, hv_input_Scale_1s, hv_input_Scale_2s, 
          hv_input_Scale_3s, hv_input_Scale_4s, hv_CaoWidthUp, hv_CaoWidthDown, hv_MaoCiWidth, 
          hv_MaoCiHeight, &hv_Result);
