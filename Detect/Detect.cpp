	...
	HTuple hv_gouchangMax, HTuple *hv_ret5)
{
	...
	{
		(*hv_ret5) = -10;
		//disp_message (hwnd, 'topfun5 最小跳动>'+a400, 'window', (Height1-400+300)/h, 20, 'red', 'false')
	}
	...
	HTuple* hv_Result, HTuple* output_ExceptionInformtion)
{
	...
	try
	{
		...
		disp_message(hv_hwnd, (((((((("粘" + fun1NgNum) + "钩宽") + fun2NgNum) + "中") + fun3NgNum) + "经") + fun4NgNum) + "震") + fun5NgNum,
	...
	HTuple* hv_Result, HTuple* output_ExceptionInformtion)
{
	...
	try
	{
		...
		disp_message(hv_hwnd, (((((("-" + top2_fun3NgNum) + "-") + top2_fun2NgNum) + "顶面") + top2_fun1NgNum) + "-") + top2_fun4NgNum,
			"window", 20, 0, "black", "true");
	...
	HTuple hv_blackArea, HTuple *hv_ret3)
{
	...
	for (hv_Index = 1; hv_Index <= 22; hv_Index += 1)
	{
		...
		if (0 != (HTuple(hv_Value>hv_blackArea).TupleOr(hv_Maxarea>(hv_blackArea / 10))))
		{
			...
			//消除旋转不居中造成的误判
			//旋转不居中与生锈类似，都有较高的灰度100+-。但旋转布局中更倾向于布满整个片，宽度大于生锈
