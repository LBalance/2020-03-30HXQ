...
////**************************************************************************************************
//用于记录各个检测函数返回不良的累积数量
HTuple fun1NgNum=0, fun2NgNum = 0, fun3NgNum = 0, fun4NgNum = 0, fun4NgButOkNum = 0;
...
void funCorrectImage(HObject ho_image, HObject *ho_imageCorrected0, HObject *ho_imageCorrected1)
{
	...
	if (0 != (hv_Channels == 1))
	{
		PaintRegion(ho_RegionDilation, ho_Image1, &(*ho_imageCorrected0), 255, "fill");
		CopyImage((*ho_imageCorrected0), &(*ho_imageCorrected1));
	}
	else if (0 != (hv_Channels == 2))
	{
		PaintRegion(ho_RegionDilation, ho_Image1, &(*ho_imageCorrected0), 255, "fill");
		PaintRegion(ho_RegionDilation, ho_Image2, &(*ho_imageCorrected1), 255, "fill");
	}
	...
	HTuple* hv_Result, HTuple* output_ExceptionInformtion)
{
	...
	HObject ho_imageCorrected0, ho_imageCorrected1;
	...
	try
	{
		...
		funCorrectImage(input_Image, &ho_imageCorrected0, &ho_imageCorrected1);
		...
		//包络线直径
		fun4(ho_imageCorrected0, &ho_region4, hv_hwnd, input_Scale_1, input_Param_BaoluoDiameterDown,
			input_Param_BaoluoDiameterUp, &hv_ret4);
		//第一次检测，如果不合格，再判断第二张
		if (0 != (hv_ret4 != 0))
		{
			//擦出第一次外径检测的显示
			DispObj(ho_imageCorrected1, hv_hwnd);
			//第二次检测
			fun4(ho_imageCorrected1, &ho_region4, hv_hwnd, input_Scale_1, input_Param_BaoluoDiameterDown,
				input_Param_BaoluoDiameterUp, &hv_ret4);
			if (0 != (hv_ret4 == 0))
			{
				//如果第二次检测合格，后续检测使用第二张图片
				CopyImage(ho_imageCorrected1, &ho_imageCorrected0);
				disp_message(hv_hwnd, "钩长度二次检测合格", "window", 120, 10, "green", "true");
				fun4NgButOkNum = fun4NgButOkNum + 1;
			}
		}
		...
		disp_message(hv_hwnd, (((((((("粘" + fun1NgNum) + "钩宽") + fun2NgNum) + "中") + fun3NgNum) + "经") + fun4NgNum) + " ") + fun4NgButOkNum,
